...
res <- paste0("20000")
exp <- paste0("786-O_786-M1A_fitHicRes_", res)
...
#=====================================================================================================
Load gencode GRCh38 annotation RData and convert to gRanges #=====================================================================================================
...
#=====================================================================================================
# Input files for analysis of 786-O and M1A 
#=====================================================================================================
...
```{r}
# Load filtered fithic data
fithic_name <- paste0(cell_line, "_merge_", res, "_25000_2500000_intraOnly.spline_pass1.res20000.FDR01.RData")
load(file = file.path(hicpath, fithic_name))
```


```{r}
# Prepare hic differentials for integration
interactions_id <- paste0("fithic_FDR01_", exp, "_", res)
hic <- hic %>% # add unique ID to all interactions
  mutate(interactionID = paste0(interactions_id, "_", row_number()))
# Convert to ginteractions
hic <- as_ginteractions(hic, keep.extra.columns = TRUE)
```
...
```{r}
# Load unique macs3 peaks
# Load macs3 peak data
macs3_peaks_H3K27ac <- read_narrowpeaks(file = file.path(macspeakpath, "M1A-H3K27ac_cmbreps_peaks_unique.narrowPeak"))
macs3_peaks_H3K27ac$name <- modify_string(macs3_peaks_H3K27ac$name)
# Prune mcols
macs3_peaks_H3K27ac <- as_tibble(macs3_peaks_H3K27ac) %>% 
  dplyr::select(seqnames, start, end, width, strand, name) %>% # select base Granges cols
  ...
  as_granges()
 # Load macs3 peak data 
macs3_peaks_H3K4me1 <- read_narrowpeaks(file = file.path(macspeakpath, "M1A-H3K4me1_cmbreps_peaks_unique.narrowPeak"))
macs3_peaks_H3K4me1$name <- modify_string(macs3_peaks_H3K4me1$name)
# Prune mcols
macs3_peaks_H3K4me1 <- as_tibble(macs3_peaks_H3K4me1) %>% 
  dplyr::select(seqnames, start, end, width, strand, name) %>% # select base Granges cols
...
generate_IDs <- function(file_path) {
  ...
  # Read in the file assuming it's a GRanges-compatible format
...
```{r}
# write out data as bedpe for visualisation in IGV
bedpe_list <- list()
# Convert hic dataframe to BEDPE format and add name and score columns
for (name in names(res_by_feature_type)) {
bedpe_df <- res_by_feature_type[[name]] %>%
  dplyr::mutate(
    chrom1 = seqnames1,
    ...
    name = paste0("fithic_integrated_", name, row_number()),        # Create a unique name for each interaction
  ) %>%
  dplyr::select(chrom1, start1, end1, chrom2, start2, end2, name) %>%
  dplyr::arrange(chrom1, start1, chrom2, start2)

bedpe_list[[name]] <- bedpe_df
}
```
...
output_file <- paste0("fithic_", name, "_", res, "_interactions.bedpe")
# Write the dataframe to a BEDPE file
write.table(bedpe_list[[name]], file = file.path(resultsDir, output_file), quote = FALSE, sep = "\t", row.names = FALSE, col.names = FALSE)
}
```
...
Retrieve the ESembl gene IDs identified through integration
...
Subest DESeq results for genes identified through integration
